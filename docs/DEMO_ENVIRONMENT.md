...
- **Display name:** **Demo – CUPE Local 79** — ⚠ *owner-authorized override.* The name uses "CUPE" (a real federation acronym) and resembles a real local, which **departs from `MOCKUP-RULES.md`** (still the standing rule: no real acronyms). Set on the project owner's explicit, informed instruction after the trademark/impersonation risk was flagged; see the deviation note atop `supabase/demo/demo_seed.sql`. `MOCKUP-RULES.md` is intentionally left unchanged — this is a documented exception, not a rule change. *Everything except the tenant name is entirely fictional.* Revert = change the one `display_name` value in the seed.
- **Structure:** a large municipal / public-sector local — Executive Board, Chief Steward, Unit Stewards, multiple bargaining units / departments, a full collective agreement, and years of grievance history. Realistic in *shape*, entirely fictional in *content*.
- **Scale (expanded seed):** ~350 members (10 named core cast + ~340 generated), 8 stewards, an 8-seat Executive Board, **16 CBA articles**, **36 grievances spanning 2015–2026** (multiple per theme: Attendance, Accommodation, Vacation, Discipline, Overtime, Seniority, Health & Safety, Scheduling, Contracting-out, Layoff/Recall, Leaves), grievance **status history**, **18 precedents** (`grievance_precedents`: how each resolved + lessons learned), **18 steward knowledge notes** (`knowledge_entries`), and a **10-document legal corpus** (CA + LOUs + arbitration awards). Interconnected so a search for a theme returns the article, the past cases, how they resolved, and the stewards' notes — the institutional-memory story, no fabricated AI.
