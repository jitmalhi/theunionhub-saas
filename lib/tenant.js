...
import { apexForHost } from '/lib/domains.js';
...
/* RESERVED_SLUGS is imported from lib/reserved-slugs.js — the single source
   shared with the middleware and /api. The DB CHECK on tenants.slug (0001)
   still mirrors it (SQL can't import JS); keep them in lockstep. */
...
function getApexDomain() {
  /* Explicit middleware injection wins; otherwise derive the apex from the
     current host via the registry (theunionhub.com / theunionhub.ca), falling
     back to .com for dev / preview hosts. */
  if (typeof window !== 'undefined' && window.__UH_CONFIG__ && window.__UH_CONFIG__.PUBLIC_BASE_DOMAIN) {
    return String(window.__UH_CONFIG__.PUBLIC_BASE_DOMAIN).toLowerCase();
  }
  const host = (typeof window !== 'undefined' && window.location && window.location.hostname) || '';
  return apexForHost(host).toLowerCase();
}

/**
 * Derive the tenant slug from a hostname.
 *
 *   theunionhub.ca            → null  (apex; marketing site)
 *   www.theunionhub.ca        → null  (reserved)
 *   local183.theunionhub.ca   → 'local183'
 *   demo.theunionhub.ca       → 'demo'
