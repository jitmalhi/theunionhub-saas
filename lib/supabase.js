...
/* Publishable anon key — RLS-safe and intentionally client-visible (see the
   "Hard rules" header: RLS is the boundary, not secrecy). This module is the
   SINGLE SOURCE OF TRUTH for the public key. Static pages (card.html,
   verify.html) and lib modules resolve it through resolveConfig() instead of
   each hardcoding their own copy. Override per environment with
   SUPABASE_ANON_KEY (env / serverless / edge) or window.__UH_CONFIG__
   (middleware injection) — both take precedence over this fallback. */
const PROD_FALLBACK_ANON_KEY = 'sb_publishable_mQ5Y9tjHHQmc9gCZPB_tHA_OwHXTc9P';
const PROD_FALLBACK_BASE_DOMAIN = 'theunionhub.com';
