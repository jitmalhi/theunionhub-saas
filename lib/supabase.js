...
/* Per-host config (Supabase project + apex) now lives in lib/domains.js — THE
   single source of truth for theunionhub.com vs theunionhub.ca. resolveConfig()
   picks the right registry entry by request host. The publishable anon keys
   there are RLS-safe and intentionally client-visible (RLS is the boundary, not
   secrecy). Override per environment with SUPABASE_URL / SUPABASE_ANON_KEY (env)
   or window.__UH_CONFIG__ (injection) — both take precedence over the registry. */
import { configForHost } from '/lib/domains.js';
...
export function resolveConfig(overrides = {}) {
  ...
  /* Region-aware fallback: pick the Supabase project + apex for THIS host
     (theunionhub.com vs theunionhub.ca) from the registry. In the browser the
     host is location.hostname; on the server (no window) this defaults to .com,
     which is harmless because env vars take precedence there anyway. */
  const host =
    (typeof window !== 'undefined' && window.location && window.location.hostname) || '';
  const hostCfg = configForHost(host);
  ...
  /* Fail loudly: never return an undefined/empty url or key. Booting with one
     would let callers fire requests at `undefined/rest/v1/...` — which hangs or
     errors opaquely. Log to the console AND throw so the misconfiguration is
     impossible to miss. (In practice the lib/domains.js registry keeps these
     populated for known hosts; this guard catches an unconfigured region — e.g.
     a .ca host before its Supabase project is filled in — or empty overrides.) */
  if (!url) {
    const msg =
      '[uh/supabase] SUPABASE_URL missing. ' +
      'Set SUPABASE_URL in .env.local, inject via middleware ' +
      '(window.__UH_CONFIG__), or pass createClient({ url }). Refusing to boot.';
    console.error(msg);
    throw new Error(msg);
  }
  if (!anonKey) {
    const msg =
      ...
      'Set SUPABASE_ANON_KEY in .env.local, inject via middleware ' +
      '(window.__UH_CONFIG__), or pass createClient({ anonKey }). Refusing to boot.';
    console.error(msg);
    throw new Error(msg);
  }
